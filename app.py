...
def index():
    lectures = [
        {"lecture_id": "lecture1",  "lecture_title": "Introduction to Software Products"},
...
def check_mcq():
    ...
    for question in lecture_data['mcqs']:
        if question['id'] == question_id:
            correct_options = [i for i, option in enumerate(question['options']) if option['is_correct']]
...
def check_tf():
    ...
    if not lecture_id or question_id is None or selected_value not in ["true", "false"]:
        return jsonify({"error": "Invalid input data"}), 400
    ...
    for question in lecture_data['true_false_questions']:
        if question['id'] == question_id:
            if question['is_true'] == (selected_value == "true"):
                return jsonify({"correct": True, "message": "Correct!"})
            else:
                return jsonify({"correct": False, "message": "Incorrect. Try again."})
    return jsonify({"error": "Question not found"}), 404
